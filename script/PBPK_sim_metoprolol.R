...
auc_obs <- round(pk.calc.auc(df$mean,sample, interval=c(sample[1],last(sample))), digits=3)
...
auc_met <- round(cbind(auc_obs,auc_all,auc_error), digits=3)
...
pk_met <- mutate(as.data.frame(pk_met), Method=c("Poulin and Theil", "Berezhkovskiy", "Rodgers and Rowland", "Schmitt", "PK-Sim Standard"))
