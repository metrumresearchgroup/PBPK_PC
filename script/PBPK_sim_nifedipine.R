...
auc_obs <- round(pk.calc.auc(df$conc,sample, interval=c(min(sample),max(sample))), digits=3)
...
auc_nif <- round(cbind(auc_obs,auc_all,auc_error), digits=3)
...
pk_nif <- mutate(as.data.frame(pk_nif), Method=c("Poulin and Theil", "Berezhkovskiy", "Rodgers and Rowland", "Schmitt", "PK-Sim Standard"))
